...
export default function SpaceManager({ spaces, onSave, onClose, t }) {
  ...
  return (
    <div style={m.overlay}>
      <div style={{ ...m.box, width:480, maxWidth:"92vw", gap:14, maxHeight:"90vh", overflowY:"auto", padding:"24px 20px" }}>
              ...
              {editId === sp.id ? (
                ...
                <>
                  <div style={{ display:"flex", alignItems:"center", gap:10, flex:1, minWidth:0 }}>
                    <div style={{ width:30, height:30, borderRadius:7, background:sp.color+"22", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>{sp.emoji}</div>
                    <span style={{ fontSize:14, fontWeight:500, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{sp.name}</span>
                  </div>
                  <button style={s.smEditBtn} onClick={() => setEditId(sp.id)}>{t.smEdit}</button>
                  {list.length > 1 && <button style={s.iconBtn} onClick={() => setList(l => l.filter(x => x.id !== sp.id))}>{"\u2715"}</button>}
                </>
              )}
            </div>
          ))}
          {adding ? (
            <div style={{ display:"flex", flexDirection:"column", gap:10, padding:12, borderRadius:8, background:"var(--bg-card)", border:"1px dashed var(--border-dashed)" }}>
            ...
            <button style={{ ...s.smCan, border:"1px dashed var(--border-dashed)", color:"var(--text-faint)", padding:10 }} onClick={() => setAdding(true)}>{t.smNew}</button>
          )}
        </div>
